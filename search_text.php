<?php
 ...
 if(!empty($_POST)){  
     ...
     FROM publicacion p inner join categoria c on p.id_categoria = c.id_categoria inner join usuario u on u.id_usuario = p.id_usuario inner join archivos a on a.publicacion=p.id_publicacion where p.texto like '%$txt%' or p.titulo like '%$txt%' or a.ruta like '%$txt%' or u.nombre like '%$txt%' and p.id_valoracion=1"); 
     $query->execute();

    $q2 = $conn->prepare("SELECT count(*) as total FROM publicacion p inner join categoria c
    on p.id_categoria = c.id_categoria
    inner join usuario u
    on u.id_usuario = p.id_usuario
    where p.texto like '%$txt%' or p.titulo like '%$txt%' or 
    p.foto like '%$txt%' or u.nombre like '%$txt%'
    and p.id_valoracion=1" );
      $q2->execute();     
    }else {
 echo "Error interno";}
?>
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  ...
  <li class="nav-item">
    <a class="nav-link active" id="usuario-tab" data-toggle="tab" href="#usuario" role="tab" aria-controls="usuario" aria-selected="false">texto</a>
  </li>
  <li class="nav-item">
